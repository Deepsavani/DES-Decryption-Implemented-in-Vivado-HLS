...
#ifndef _des_dec_sub_key_H_
#define _des_dec_sub_key_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "des_dec_sub_key_memcore.h"

SC_MODULE(des_dec_sub_key) {
...
protected:
    ...
    des_dec_sub_key_memcore* des_dec_sub_key_memcore_U;
...
public:
    ~des_dec_sub_key();
    SC_CTOR(des_dec_sub_key) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        des_dec_sub_key_memcore_U = new des_dec_sub_key_memcore("des_dec_sub_key_memcore_U");
        des_dec_sub_key_memcore_U->clk      ( clk );
        des_dec_sub_key_memcore_U->reset    ( reset );
        des_dec_sub_key_memcore_U->ce0      ( i_ce0 );
        des_dec_sub_key_memcore_U->we0      ( i_we0 );
        des_dec_sub_key_memcore_U->address0 ( memcore_iaddr );
        des_dec_sub_key_memcore_U->d0       ( i_d0 );
        des_dec_sub_key_memcore_U->q0       ( i_q0 );

        des_dec_sub_key_memcore_U->ce1      ( t_ce0 );
        des_dec_sub_key_memcore_U->we1      ( t_we0 );
        des_dec_sub_key_memcore_U->address1 ( memcore_taddr );
        des_dec_sub_key_memcore_U->d1       ( t_d0 );
        des_dec_sub_key_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "des_dec_sub_key.trace");
            // ports
